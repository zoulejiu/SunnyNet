...
func (t *transferWriter) writeHeader(w io.Writer, trace *httptrace.ClientTrace) error {
	ConnectionLength := t.Header["connection"]
	if len(ConnectionLength) > 0 {
	...
	if t.shouldSendContentLength() {
		length := t.Header["content-length"]
		if len(length) > 0 {
		...
		length := t.Header["transfer-encoding"]
		if len(length) > 0 {
	...
	if t.Trailer != nil {
		...
		for k := range t.Trailer {
			...
			switch k {
			case "Transfer-Encoding", "Trailer", "Content-Length", "transfer-encoding", "trailer", "content-length":

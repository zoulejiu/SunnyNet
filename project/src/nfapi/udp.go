...
func (p *udpConnection) SendServer(data []byte) bool {
	...
	if p.Send != nil {
		r, _ := Api.NfUdpPostSend(p.Send.Id, p.Send.RemoteAddress, data, p.Send.options)
		return r == 0
	}
	if p.Conn == nil {
		return false
...
func (p *udpConnection) SendClient(data []byte) bool {
	...
	if p.Receive != nil {
		r, _ := Api.NfUdpPostSend(p.Receive.Id, p.Receive.RemoteAddress, data, p.Receive.options)
		return r == 0
	}
